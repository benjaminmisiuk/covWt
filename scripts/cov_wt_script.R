...
for(l in buff_width){
  ...
  for(k in ind_pts){
    ...
    for(j in 1:100){
      ...
      result_i$cor_oob_wt = cov.wt(data.frame(val[ ,1], val[ ,2]), wt = val[ ,3], cor = TRUE)$cor[2]
      result_i$mse_oob_wt = mse_wt(val[, 1], val[, 2], val[, 3])
      result_i$rmse_oob_wt = rmse_wt(val[, 1], val[, 2], val[, 3])
      result_i$ve_oob_wt = ve_wt(val[, 1], val[, 2], val[, 3])
    ...
    save(results, file = paste0(out, paste('covwt', n, n_clust, l, k, n-k, '.RData', sep = '_')))
  }
}
